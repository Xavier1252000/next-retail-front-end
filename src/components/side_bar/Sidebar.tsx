...
interface HeadModule {
  id: string; // added ID for unique tracking
  name: string;
  to: string;
  icon?: string;
...
export default function Sidebar() {
  ...
  useEffect(() => {
    if (!loading && response?.response?.data?.userPermissions) {
      ...
      const formattedPermissions: HeadModule[] = permissions.map(
        (perm: any, index: number) => ({
          id: `module-${index}`, // unique ID
          ...
          {userPermissions.map((module) => {
            ...
            return (
              <li key={module.id}>
                ...
                {/* Submodules */}
                <AnimatePresence>
                  {openModules[module.id] && hasSubModules && (
                    <motion.ul
